...
class _PatientsPageState extends State<PatientsPage> {
  ...
  // ---------------------------------------------------------------------
  // تحويل النص العربي لصورة بسُمك عريض وتقطعيها لأوامر ESC * m=33
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // إرسال البيانات للبلوتوث مقسمة لتجنب الازدحام
  // ---------------------------------------------------------------------
  ...
  Future<void> _printReceipt() async {
    ...
    try {
      ...
      // --- Header ---
      finalBytes.addAll([0x1B, 0x61, 0x01]);
      finalBytes.addAll([0x1B, 0x21, 0x30]);
      finalBytes.addAll("S-LAB\n".codeUnits);
      finalBytes.addAll("01009726362\n".codeUnits);
      finalBytes.addAll([0x1B, 0x21, 0x00]);
      finalBytes.addAll("01009726362\n".codeUnits);
      finalBytes.addAll("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n".codeUnits);

      // --- Patient Info ---
      finalBytes.addAll([0x1B, 0x61, 0x00]);
      ...
      final tubeCmds = await _renderArabicTextToEscStarCommands(
        "$patientIdStr - ${_nameController.text} - $patientIdStr - ${_nameController.text}",
        fontSize: 22,
      );
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              // --- الجزء المُضاف: عرض البطاقات الخاصة بالتحاليل المختارة ---
              if (_selectedAnalysisIds.isNotEmpty) ...[
                Wrap(
                  spacing: 8.0,
                  runSpacing: 4.0,
                  children: _selectedAnalysisIds.map((id) {
                    final item = _allAvailableAnalysis.firstWhere(
                      (element) => element['id'] == id,
                      orElse: () => {'name': 'غير معروف', 'price': 0},
                    );
                    return Chip(
                      label: Text(
                      ...
                      onDeleted: () {
                        setState(() {
                          _selectedAnalysisIds.remove(id);
                          _calculateFinances();
                        });
                      },
